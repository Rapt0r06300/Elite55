...
def current_ranking_mode() -> str:
    return _normalize_mode(_ranking_mode_ctx.get())


def ranking_payload() -> dict[str, str]:
    mode = current_ranking_mode()
    meta = RANKING_META[mode]
    return {
        "ranking_mode": mode,
...
def _route_sort_key(route: dict[str, Any], mode: str) -> tuple[float, ...]:
...
def _loop_sort_key(loop: dict[str, Any], mode: str) -> tuple[float, ...]:
...
def sort_routes_by_mode(routes: list[dict[str, Any]] | None, mode: str | None = None) -> list[dict[str, Any]]:
    selected_mode = _normalize_mode(mode or current_ranking_mode())
    return sorted(list(routes or []), key=lambda row: _route_sort_key(row, selected_mode), reverse=True)


def sort_loops_by_mode(loops: list[dict[str, Any]] | None, mode: str | None = None) -> list[dict[str, Any]]:
    selected_mode = _normalize_mode(mode or current_ranking_mode())
    return sorted(list(loops or []), key=lambda row: _loop_sort_key(row, selected_mode), reverse=True)
...
if elite_main is not None:
    ...
    if not getattr(elite_main.app.state, "elite55_ranking_middleware_installed", False):
        ...
        async def elite55_ranking_mode_middleware(request: Any, call_next: Any) -> Any:
            ...
            if not mode and request.method.upper() in {"POST", "PUT", "PATCH"}:
        ...
        elite_main.app.state.elite55_ranking_middleware_installed = True
