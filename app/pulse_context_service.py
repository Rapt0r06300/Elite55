...
def build_sources_payload(elite_main: Any) -> dict[str, Any]:
...
def build_engine_status_from_values(
    ...
    original = getattr(elite_main, "_elite55_original_build_engine_status_from_values", None)
    if callable(original):
        return original(rows, name_summary, local_status, current_market, current_system)
    if rows < 500:
        phase = "bootstrapping"
...
def install_pulse_context_service_patches(elite_main: Any) -> None:
    ...
    elite_main._elite55_original_build_engine_status_from_values = getattr(elite_main, "build_engine_status_from_values", None)
    elite_main._elite55_original_build_engine_status = getattr(elite_main, "build_engine_status", None)
